...
class ProviderDoctorTests(unittest.TestCase):
    def test_build_provider_doctor_report_includes_sidecar_env_file_hints(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(paddle["support_tier"], "stable")
            self.assertEqual(paddle["validated_platforms"], ["windows"])
            self.assertEqual(paddle["recommended_entrypoint"], "no-watermar batch apply")
    ...
    def test_build_provider_doctor_report_includes_compatibility_matrix_and_version_status(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(report["compatibility_matrix"]["paddleocr"]["support_tier"], "stable")
            self.assertEqual(report["compatibility_matrix"]["diffusers_inpaint"]["support_tier"], "experimental")
            paddle = next(sidecar for sidecar in report["sidecars"] if sidecar["provider_name"] == "paddleocr")
